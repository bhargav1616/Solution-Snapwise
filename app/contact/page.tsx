...
export default function ContactPage() {
  ...
  try {
    // 👇 FormData object banavo
    const formPayload = new FormData();
    formPayload.append("name", formData.name);
    formPayload.append("email", formData.email);
    formPayload.append("phoneNumber", formData.phoneNumber);
    formPayload.append("projectDetails", formData.projectDetails);
    formPayload.append("hireDeveloper", formData.hireDeveloper);
    if (formData.file) {
      formPayload.append("resume", formData.file); // 👈 resume attach
    }
    formPayload.append("captcha", String(formData.captcha));

    const res = await fetch("/api/contact", {
      method: "POST",
      body: formPayload, // 👈 JSON.stringify() na thay, direct FormData moklo
    });
